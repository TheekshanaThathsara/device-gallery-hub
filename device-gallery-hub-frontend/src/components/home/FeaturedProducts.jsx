...
export default function FeaturedProducts() {
  ...
  const featuredProducts = [
    ...
    },
    {
      ...
      price: 89.99,
      oldPrice: 119.99,
      image: 'https://images.unsplash.com/photo-1589132928127-10179f294b02?ixlib=rb-1.2.1&auto=format&fit=crop&w=600&q=80',
      badge: 'Professional',
      rating: 4.7,
      reviews: 68
    },
    ...
    },
    {
      ...
      price: 79.99,
      oldPrice: 99.99,
      image: 'https://images.unsplash.com/photo-1546435770-a3e426bf472b?ixlib=rb-1.2.1&auto=format&fit=crop&w=600&q=80',
      badge: 'Innovative',
      rating: 4.5,
      reviews: 98
    }
  ...
  // Initialize with stock status for all products when component mounts
  useEffect(() => {
    const productsWithStock = featuredProducts.map(product => {
      const stockStatus = Math.random() > 0.2 ? 'In Stock' : 'Low Stock';
      return {...product, stockStatus};
    });
    
    setAnimatedProducts(productsWithStock);
    setIsLoading(false); // Ensure loading state is turned off after products are loaded
    
    // Add a slight delay before triggering the fade-in effect
    setTimeout(() => {
      setFadeIn(true);
    }, 100);
  }, []);
  ...
  useEffect(() => {
    // Reset fade-in and show loading state when category changes
    setFadeIn(false);
    setIsLoading(true);
  ...
  const filteredProducts = activeCategory === 'all' 
    ? animatedProducts 
    : animatedProducts.filter(product => product.category === activeCategory);
  ...
  useEffect(() => {
    console.log('Current state:', {
      ...
      animatedProductsCount: animatedProducts.length,
      isLoading,
      fadeIn,
      // More detailed info
      hasFilteredProducts: filteredProducts.length > 0,
      hasAnimatedProducts: animatedProducts.length > 0,
      sampleProduct: animatedProducts.length > 0 ? animatedProducts[0] : null
    });
  }, [activeCategory, filteredProducts.length, animatedProducts.length, isLoading, fadeIn]);
    ...
    <section className="py-16 bg-gray-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex justify-center mt-10 mb-8">
          <div className="inline-flex flex-wrap justify-center gap-2 p-2 bg-white rounded-2xl shadow-md border border-gray-50">
            {categories.map(category => {
              ...
              return (
                <button
                  ...
                  className={`px-5 py-2.5 text-sm font-medium rounded-xl transition-all duration-300 flex items-center gap-2 ${
                    isActive
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {isLoading ? (
            // Loading skeletons
            [...Array(8)].map((_, index) => (
              <div key={`skeleton-${index}`} className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100 animate-pulse">
                {/* Image skeleton */}
            ...
            filteredProducts.map((product, index) => (
              <div 
                key={product.id} 
                className={`group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-blue-200 transform hover:-translate-y-2 relative ${fadeIn ? 'opacity-100' : 'opacity-0'}`}
                style={{ 
              ...
              <div className="relative overflow-hidden">
                <img 
                  ...
                  onError={(e) => {
                    ...
                    e.target.src = 'https://via.placeholder.com/600x400?text=' + product.category;
                  }}
                  loading="lazy"
                />
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  {/* Rating */}
                  <div className="flex items-center bg-yellow-50 px-2 py-1 rounded-md shadow-sm">
                    <svg className="w-4 h-4 text-yellow-500 fill-current" viewBox="0 0 20 20">
                      <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                ...
                <div className="relative">
                  ...
                  {(product.rating > 4.7 && product.reviews > 200) && (
                    <div className="absolute -right-2 -top-2 flex items-center">
                      <svg className="w-5 h-5 text-yellow-400 filter drop-shadow" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    </div>
                  )}
                ...
                {/* Stock status */}
                <div className="mb-4">
                  <span className={`text-xs font-semibold rounded-full px-3 py-1.5 inline-block ${product.stockStatus === 'In Stock' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>
                    {product.stockStatus}
                  </span>
                </div>

                {/* Add to Cart Button */}
                <button className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-medium py-3 px-4 rounded-xl transition-all duration-300 transform hover:scale-[1.02] hover:shadow-lg flex items-center justify-center group">
            ...
            <div className="col-span-1 sm:col-span-2 lg:col-span-4 text-center py-16">
              ...
              <div className="p-6 md:p-8 flex flex-col">
                ...
                <div className="mt-auto">
                  <div className="grid grid-cols-2 gap-3">
                    <button className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-medium py-3 px-4 rounded-xl transition-all duration-300 flex items-center justify-center">
                      <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                      </svg>
                      Add to Cart
                    </button>
                    <button className="flex-1 border border-blue-600 text-blue-600 hover:bg-blue-50 font-medium py-3 px-4 rounded-xl transition-all duration-300 flex items-center justify-center">
                      <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
